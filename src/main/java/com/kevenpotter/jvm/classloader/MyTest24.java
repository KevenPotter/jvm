 ...
 * 这就改变了父ClassLoader不能使用子ClassLoader或是其他没有直接父子关系的ClassLoader加载的类的情况,即改变了双
 * 亲委托模型.
 * <p>
 * 线程上下文类加载器就是当前线程的Current ClassLoader.
 * 在双亲委托模型下,类加载是由下至上的,即下层的类加载器会委托上层进行加载.但是对于SPI来说,有些接口是Java核心库所
 * 提供的,而Java核心库是由启动类加载器来加载的,而这些接口的实现却来自于不同的jar包(厂商提供),Java启动类加载器是
 * 不会加载其他来源的jar包,这样传统的双亲委托模型就无法满足SPI的要求.而通过给当前线程设置上下文类加载器,就可以由
 * 设置的上下文类加载器来实现对于接口实现类的加载.
 */
